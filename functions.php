...
function navigation_custom_footer_content() {
	genesis_widget_area(
		'footer-widget-left',
		[
			'before' => '<div class="footer-widgets-left widget-area">',
			'after'  => '</div>',
		]
	);
...
function navigation_register_sidebars() {
	...
	genesis_register_sidebar(
		[
			...
			'name'        => __( 'Footer-Left', 'navigation-pro' ),
			'description' => __( 'Add social links or other information important for your site.', 'navigation-pro' ),
		]
	);
...
// Exclude Testimonials from Blog archive
...
/** Add static image above blog archives via widget - Solution is_home() provided by Erik D. Slater in FB Genesis WP group */
...
add_action( 'genesis_before_loop', 'blog_archive_image' );
function blog_archive_image() {
